...
# Minimum score thresholds per ngram size, matching the three scoring tiers
# in build_knowledge_base.py: n<=2 (freq-dominant), n==3 (balanced), n>=4 (length-dominant)
SCORE_THRESHOLD_BY_NGRAM = {
    1: 6.0,   # n<=2: log-amplified scores are much higher; raise bar accordingly
    2: 5.0,
    3: 4.0,   # n==3: balanced tier, moderate threshold
    4: 3.5,   # n>=4: length-dominant, lower threshold is fine
    5: 3.0,
}

def get_score_threshold(ngram_size):
    return SCORE_THRESHOLD_BY_NGRAM.get(ngram_size, 3.5)
...
def phrase_redundancy_coverage(phrase, knowledge_base, ngram_size):
    ...
    # to be considered worth flagging (one strong word in a 5-gram is meaningful).
    words = normalize_phrase(phrase).split()
    if not words:
        return 0
    word_threshold = get_score_threshold(1)  # always judge individual words by n=1 tier
    redundant_count = sum(
        1 for word in words
        if get_single_word_score(word, knowledge_base) >= word_threshold
    )
...
def detect_knowledge_matches(prompt, knowledge_base):
    ...
    for phrase, data in knowledge_base.items():
        ...
        # Filter by tier-aware minimum score before coverage check
        if score < get_score_threshold(ngram_size):
            continue

        if ngram_size > 1:
            if not phrase_redundancy_coverage(normalized, knowledge_base, ngram_size):
                continue
